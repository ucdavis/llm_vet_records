...
generate_combined_tile_plot <- function(df) {
  ggplot(df, aes(x = respondent_no, y = case)) +
    ...
    scale_fill_manual(
      values = c("TRUE" = "#3282Bd", "FALSE" = "#FA8533", "missing" = "white"), 
      na.value = "white",
      breaks = c("TRUE", "FALSE"), 
      labels = c("TRUE", "FALSE")
    ) +
...
### Changes
Fixed that the NA value is displayed as white by changing it to "missing" before filtering out NA values
...
problem_questions_temp0 <- problem_questions_temp0 %>%
  mutate(across(starts_with("response_"), as.character)) %>%
  mutate(across(starts_with("response_"), ~ replace_na(., "missing")))
