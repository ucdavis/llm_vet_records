...
plot_invalid_output <- function(llm_response_parsed) {
  ...
  error_plot <- ggplot(error_type_data, aes(x = factor(temperature), y = percentage, fill = status)) +
    ...
    labs(x = "Temperature", y = "Invalid output (%)", fill = "") +
    scale_fill_manual(values = output_color_scale,  labels = output_label, na.translate = FALSE) +
    theme_bw() +
...
ggsave(filename = png_file_path, plot = tuning_GPT4o_invalid_output_plot, width = 180, height = 45, units = "mm", dpi = 300)
