...
categorized_errors <- categorized_errors %>% 
  mutate(disagreement_type_pct = case_when(
    disagreement_type == "Citation" ~ "Citation\n(25.6%)",
    disagreement_type == "Interpretation" ~ "Interpretation\n(74.4%)"
  ),
  ambiguity_pct = case_when(
    ambiguity == "Temporal" ~ "Temporal\n(55.8%)",
    ambiguity == "Qualitative" ~ "Qualitative\n(34.9%)",
    ambiguity == "None" ~ "None (9.3%)"
  ),
  agreement_category_pct = case_when(
    agreement_category == "0" ~ "0 (18.6%)",
    agreement_category == "1" ~ "1 (30.2%)",
    agreement_category == "2" ~ "2 (51.2%)"
  ))
...
alluvian_plot <- ggplot(categorized_errors,
       aes(axis1 = agreement_category_pct, 
           axis2 = disagreement_type_pct, 
           axis3 = ambiguity_pct)) +
  geom_alluvium(aes(fill = agreement_category)) +
  scale_fill_manual(
      values = human_agreement_palette) +
  geom_stratum() +
  geom_text(stat = "stratum",aes(label = after_stat(stratum)), angle=c(0,0,0,45,45,45, 45, 0),size=2.5) +
  scale_x_discrete(limits = c("Divergent human opinions (n)", "Type of discrepancy", "Ambiguity of electronic health record")) +
  labs(y = "Count") +
  theme_bw() +
  theme(
